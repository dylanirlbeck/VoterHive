...
func isNumber(_ astring: String) -> Bool {
    for character in astring.unicodeScalars {
...
func remove4LetterWords(from swearArray: [String]) -> [String] {
    var cleanArray = [String]()
    for word in swearArray {
        if word.count != 4 {
...
remove4LetterWords(from: dirtyWordsArray)
...
class MovieArchive {
    func filterByDirector(movies: Dictionary<String,String>, director: String) -> [String] {
        var returnMovies = [String]()
        for (title, movieDirector) in movies {
            if movieDirector == director {
                returnMovies.append(title)
            }
        }
        return returnMovies
    }
}
var thisArchive = MovieArchive()
thisArchive.filterByDirector(movies: movies, director: "Christopher Nolan")
